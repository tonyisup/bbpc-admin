...
import { Trash2, Search, Film, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react";
...
type MovieSortKey = "title" | "year" | "reviews";

interface SqlTmdbMovie {
  id: number;
  title: string;
  release_date: string;
...
const MoviesPage: NextPage = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: MovieSortKey; direction: 'asc' | 'desc' | null }>({
    key: 'title',
    direction: 'asc'
  });
  ...
  const handleAdd = (result: SqlTmdbMovie) => {
    addMutation.mutate({
      title: result.title,

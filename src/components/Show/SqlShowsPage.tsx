...
import { Trash2, Search, Film } from "lucide-react";
...
const ShowsPage: NextPage = () => {
  ...
  const handleAdd = (result: SqlTmdbShow) => {
    const releaseYear = getPlainDateYear(result.release_date) ?? 0;
    addMutation.mutate({
      title: result.title,
      year: releaseYear,
